...
import algosdk, { makeApplicationCreateTxn } from "algosdk";
import { algoClient } from "./api/algoClient";
...
const compileProgram = async (programSource: string) => {
  ...
  let compileResponse = await algoClient().compile(programBytes).do();
 ...
 */
async function awaitForConfirmation(txId: any) {
...
export async function sendRawTransaction(
  ...
  await awaitForConfirmation(txId);
...
export async function executeTransaction(
  ...
  return await sendRawTransaction(signedTxn);
}
 ...
 */
export async function executeGroupTransaction(
...
export async function submitSignedTransactions(
  ...
  const response = await sendRawTransaction(stxnsUint8Array);
  return response;
}
...
export async function appOptIn(
  ...
  const stxns = await executeGroupTransaction([txn], [undefined]);
...
export async function hasOptedIn(
  ...
  const appInfo = await getAppInfoInUserContext(walletAddr, appIndex);
 ...
 */
export async function testAppOptIn(appId: number, wallet: Wallet): Promise<any> {
  const isOptedIn = await hasOptedIn(wallet.addr, appId);
  if (isOptedIn) {
    return;
  }
  const suggestedParams = await algoClient().getTransactionParams().do();
  ...
  const stxns = await executeGroupTransaction([txn], [wallet]);

  await submitSignedTransactions(stxns);
...
export async function assetOptIn(
  ...
  const stxns = await executeTransaction(txnAssetOptIn, wallet);
  return stxns;
}
...
export async function assetTransfer(
  ...
  const stxns = await executeGroupTransaction([transferTx], [from_wallet]);
  return stxns;
}
 ...
 */
export async function assetTransferWithoutSigning(
  from_wallet: any,
  to_addr: string,
  assetId: number,
  amount: number
 ...
 */
export async function assetsTransfer(
  from_wallet: Wallet,
  to_addr: string,
  ...
  for (const asset of assets) {
    ...
    const transferAssetPromise = assetTransfer(
      from_wallet,
      to_addr,
      assetIndex,
      amount
 ...
 */
export async function optInAssetsAsGroupTransaction(
  assets: number[],
  walletAddr: string,
  ...
  const stxns = await executeGroupTransaction(
    assetOptInTxns,
 ...
 */
export async function getApplicationInfo(index: number) {
  const appInfo = await algoClient().getApplicationByID(index).do();
  return appInfo;
}
 ...
 */
export async function getBoxValueForApplication(
  appIndex: number,
  boxName: string
) {
 ...
 */
export async function getGlobalStateValue(index: number, b64VarName: string) {
  const info = await getApplicationInfo(index);
...
export async function getAppInfoInUserContext(
  ...
  const info = await getWalletInfo(walletAddr);
 ...
 */
export async function getApplicationWalletInfo(index: number) {
  const appWallet = algosdk.getApplicationAddress(index);
  const walletInfo = getWalletInfo(appWallet);
  return walletInfo;
}
 ...
 */
export async function getForeignAssetsArray(index: number) {
  // assets
  const info = await getApplicationWalletInfo(index);
 ...
 */
export function generateAssetByteArray(assets: number[]): Uint8Array {
  const sizeOf = 8;
 ...
 * @returns {Uint8Array} encoded asset array as a Uint8Array
 */
export function encodeAssetArrayToUint8Array(assets: number[]): Uint8Array {
  const assetByteArray = generateAssetByteArray(assets);
...
export async function createApplication(
  ...
  const [approvalProgram, clearProgram, params] = await Promise.all([
    compileProgram(approvalSource),
    compileProgram(clearSource),
  ...
  const stxns = await executeGroupTransaction([txn], [creatorWallet]);
 ...
 */
export async function createApplicationWithoutSigning(
  approvalSource: string,
  clearSource: string,
  config: CreateApplicationConfig,
  creatorAddr: string,
 ...
 */
export async function commonApplicationClaimFees(
  ...
  const stxns = await executeGroupTransaction([appTxn], [serverWallet]);
  const response = await submitSignedTransactions(stxns);
 ...
 * @returns {Promise<Record<string, any>>} transaction response
 */
export async function commonApplicationOptIntoAsset(
  serverWallet: FalseyWallet,
  assetIndex: number,
 ...
 * @returns {Promise<any[]>} array of transactions
 */
export async function commonApplicationOptIntoAssets(
  serverWallet: FalseyWallet,
  assetIndices: number[],
  optInAssetFcnName: string,
  appIndex: number
  ...
  for (const assetIndex of assetIndices) {
    const promise = commonApplicationOptIntoAsset(
      serverWallet,
      assetIndex,
      optInAssetFcnName,
      appIndex
...
export async function sendAlgo(
  ...
  const stxns = await executeGroupTransaction([paymentTxn], [serverWallet]);
  return stxns;
}
 ...
 */
export async function sendAlgoWithoutSigning(
  serverWallet: FalseyWallet,
  amount: number,
  address: string
) {
 ...
 * @returns {Promise<Record<string, any>>} transaction response
 */
export async function assetTransferFromContract(
  appIndex: number,
  assetIndex: any,
  amount: number,
  wallet: Wallet
  ...
  const stxns = await executeGroupTransaction([transferTx], [wallet]);
 ...
 * @returns {Promise<any[]>} array of transactions
 */
export async function deleteApplication(appIndex: number, wallet: Wallet) {
 ...
 */
export async function getOptedInApplications(appId: number): Promise<number[]> {
...
export function makeWallet(
  addr: string,
  sk: Uint8Array | undefined = undefined
): Wallet {
  return { addr, sk };
}

export function getARC69MimetypeFromMediaMimeType(mimeType: string | undefined): string {
